...
example {n : ℤ} (hn : n ≥ 5) : n ^ 2 > 2 * n + 11 :=
  calc
    n ^ 2
      = n * (n - 2) + 2 * n := by ring
    _ ≥ 5 * (5 - 2) + 2 * n := by rel [hn]
    _ = 2 * n + 11 + 4 := by ring
    _ > 2 * n + 11 := by extra -- `extra`: inequality has extra positive pieces so it holds.
                               -- `numbers`: doesn't work here because it has to be about pure numbers.
...
example {a b : ℚ} (h1 : a ≥ 0) (h2 : b ≥ 0) (h3 : a + b ≤ 8) :
  ...
  calc
    ...
    _ ≤ 2 * (8 * b) + 8 * a + a := by rel [h3] -- Nonnegativity of a and b matters here.
    _ = 7 * b + 9 * (a + b) := by ring
    _ ≤ 7 * b + 9 * 8 := by rel [h3]
    _ = 7 * b + 72 := by ring
