...
example {a b : ℚ} (h1 : a - 3 = 2 * b) : a ^ 2 - a + 3 = 4 * b ^ 2 + 10 * b + 9 :=
  calc
    ...
    _ = a ^ 2 - 2 * b := by rw [h1]
    _ = a ^ 2 - 6 * (2 * b) + 10 * b := by ring
    _ = a ^ 2 - 6 * (a - 3) + 10 * b := by rw [h1]
    _ = (a - 3) ^ 2 + 10 * b + 9 := by ring
    _ = (2 * b) ^ 2 + 10 * b + 9 := by rw [h1]
    _ = 4 * b ^ 2 + 10 * b + 9 := by ring

-- Example 1.3.10
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = (z ^ 2 - 2) ^ 2 + 3 * (z ^ 2 - 2) - z * (z ^ 2 - 2) + 3 := by ring
    _ = 0 ^ 2 + 3 * 0 - z * 0 + 3 := by rw [h1]
    _ = 3 := by ring
